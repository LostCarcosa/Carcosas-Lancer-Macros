/* 
Lancer - Quick Combat Builder V1
...
Tested on Foundry 10.291, Lancer 1.5.4  
...
V1 build may be somewhat buggy. I have tested it on four combats from Operation Solstice Rain with no major issues.
...
- Certain optionals are 'linked'- like Sniper's Defensive Grapple/Relocation- but there's no 
obvious way for foundry to know that it should grab these as a package. Will consider.
...
let classComp = game.packs.get("world.npc_class")
await classComp.getDocuments()
let classNames = []
let classIds = []

for(let item of classComp){
...
async function createClassList(){
	let classPack = game.packs.get("world.npc_class")
...
async function createTemplateList(){
	let templatePack = game.packs.get("world.npc_template")
...
async function processSettings(str){
	...
	let file_list = await FilePicker.browse("data", arr[1][1]).then(picker => picker.files);
...
function processSingleNPC(npc_text, classList, templateList, settings){
	...
	//optionals should look like "Kinetic Compensation, Legendary, Quick March" right now.
...
async function buildSingleNPC(npc_data, folder_id){
	...
	await mech.update({"system.hp": mech.system.derived.hp.max}) //for some reason this... doesn't really work- it seems like it takes a nonzero amount of time for the max hp to update and this code runs too quickly. hm.
...
async function populateClass(actor, npc_data){
	...
	/* Thanks to Eranziel for pointing me toward the on_root_drop function, which has greatly simplified this code */
	
	let classMM = await classItem.sheet.getDataLazy() // yeah I don't know why either
	classMM = classMM.mm
	await actor.sheet.on_root_drop(classMM) //this emulates drag-drop behavior, which autopopulates the base templates, sets mech images, fills to max hp, etc.
	...
	/* THIS CODE IS NOW OBSELETE since we can now use drag-drop.
	let baseFeatures = classItem.system.base_features
	console.log("Base features are " + baseFeatures.map(feat => feat.id))
	console.log(featureComp)
	
	let baseFeaturesArr = baseFeatures.map(feat => featureComp.get(feat.id)) //go into the features compendium and snatch the literal items
	console.log(baseFeaturesArr)
	
	actor.createEmbeddedDocuments("Item", baseFeaturesArr)
	*/
...
async function populateOptionals(actor, ctItem, optionals){
	...
	let optionalFeatures = ctItem.system.optional_features
	
	
	let unmatched_optionals= [] //compile a list of optionals that have not been matched. This is easier than trimming an array, imo.
	let optIds = []
	let optFeatureNames = optionalFeatures.map(f => getOptFeatName(f.id)) // unfortunately the class does not contain the NAMES of the optional systems, just their ids, so in order to have proper lookup we need to build up the corresponding names array.
	
	//console.log("optional feature names are " + optFeatureNames)
	...
	for(let opt of optionals){
		...
		for(let i = 0; i < optionalFeatures.length; i++){
			if(compareStringContents(opt, optFeatureNames[i])){
				found = true;
				optIds.push(optionalFeatures[i].id)
				break;
			}
		}
		if(!found){
	...
	optFeatureArr = optIds.map(ids => featureComp.get(ids))
...
function getOptFeatName(optFeatId){
	return featureComp.get(optFeatId).name
}
...
async function populateTemplate(actor, npc_data, templateName){
	...
	/* as before- obselete due to drag-drops. 
	let baseFeatures = templateItem.system.base_features
